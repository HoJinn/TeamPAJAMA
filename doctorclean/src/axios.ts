...
export interface DefSuccess204 extends ResultType<undefined> {
  status: 204;
  data: null;
  success: true;
  // err: undefined;
...
const defSuccess = <T = any>(res: AxiosResponse<T>): DefSuccess<T> | DefSuccess204 => {
  const { status, data } = res;
  if (status == 204) {
    return { status: 204, data: null, success: true, err: undefined }
  } else
