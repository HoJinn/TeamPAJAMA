...
import { dataSizeParse } from 'src/format';
...
const NoticeDetail: NextPage = () => {
  ...
  const filesItem = contentData && contentData.attachments ? contentData.attachments.map((file, i) => <Box key={i} alignItems="center" display='flex' marginTop={3}>
    <a className={classes.linkBtn} target="__blank" href={`${process.env.NEXT_PUBLIC_API_IMG}/${file.id}`}>
      <span>
        <AttachFileIcon className={classes.fileIcon} />
        <span className={classes.hide}>첨부파일</span>
      </span>
      <Typography variant='body1' className={classes.fileTxt}>{file.originName} ({dataSizeParse(file.size)})</Typography>
    </a>
  </Box>) : undefined
  ...
  return (
    <Layout>
      <Head>
        <title>다온븟 공지사항 - {contentData?.title || "상세보기"}</title>
      </Head>
      <TitleBox subTitle='Notice' title="공지사항" img={Bg} size={taFlag ? "MD" : 'SM'} />
